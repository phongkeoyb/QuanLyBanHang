...
namespace QL_BanHang.View
{
    public partial class frmHoaDon : Form
    {
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                if (ctCtr.DelData(txtMa.Text.Trim()))
                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if(txtMa.Text == null)
            {
                MessageBox.Show("Chưa nhập mã hóa đơn", "Lỗi");
            }
            else
            {
                string mahd = txtMa.Text;
                ctCtr.GetData(mahd);
                dtgvDSHH.DataSource = ctCtr.GetData(mahd);
            }
        }
        ...
        private void dtgvDSHD_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                ...
                txtMa.Text = dtgvDSHD.Rows[e.RowIndex].Cells["MaHD"].Value.ToString();
                txtNgayLap.Text = dtgvDSHD.Rows[e.RowIndex].Cells["NgayLap"].Value.ToString();
            }
        }
    }
}
